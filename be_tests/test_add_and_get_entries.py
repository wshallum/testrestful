...
class TestAddAndGetEntries(BaseBackendTest):
    ...
    def test_add_and_get_entries(self):
        ...
        self.assertEqual(all_entries.status_code, 200, "GET /entries returns 200")
        # The user then adds an entry
        add_result = requests.post(self.url('/entries'), json.dumps(dict(name="Joe")))
        self.assertEqual(add_result.status_code, 201, "POST /entries to create returns 201")
